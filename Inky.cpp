...
void Inky::load(){
	...
	glPushMatrix();
		...
		//glColor3f(Inky_COLOR_R,Inky_COLOR_G,Inky_COLOR_B);
		if(state <= GHOST_STATE_NORMAL){
			matBody[0] = INKY_COLOR_R;
			matBody[1] = INKY_COLOR_G;
			matBody[2] = INKY_COLOR_B;
		}
		else{
			matBody[0] = 1.0;
			matBody[1] = 1.0;
...
float Inky::getXPoz(){return xpoz;}
float Inky::getYPoz(){return ypoz;}
int* Inky::getState(){return &state;}
void Inky::kill(){
	jail_time = 400;
	jail = true;
	xpoz = PINKY_START_X;
...
void Inky::move(int** matrix2D, int pacman_x, int pacman_y){
...
void Inky::moveJail(int** matrix2D){
	if (jail_time == 0 && jail){
...
void Inky::moveChase(int** matrix2D, int pacman_x, int pacman_y){
	float x = pacman_x - xpoz;
	float y = pacman_y - ypoz;
...
void Inky::moveLeft(int** matrix2D){
	xpoz = xpoz-ACT_RATE;
}

void Inky::moveRight(int** matrix2D){
	xpoz = xpoz+ACT_RATE;
}

void Inky::moveUp(int** matrix2D){
	ypoz = ypoz+ACT_RATE;
}

void Inky::moveDown(int** matrix2D){
	ypoz = ypoz-ACT_RATE;
}
void Inky::timer(){
	if(state > 0)
