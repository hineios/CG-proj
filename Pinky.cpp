...
void Pinky::load(){
	...
	glPushMatrix();
		...
		if(state <= GHOST_STATE_NORMAL){
			matBody[0] = PINKY_COLOR_R;
			matBody[1] = PINKY_COLOR_G;
			matBody[2] = PINKY_COLOR_B;
		}
		else{
			matBody[0] = 1.0;
			matBody[1] = 1.0;
...
float Pinky::getXPoz(){return xpoz;}
float Pinky::getYPoz(){return ypoz;}
int* Pinky::getState(){return &state;}
void Pinky::kill(){
	jail_time = 400;
	jail = true;
	xpoz = PINKY_START_X;
...
void Pinky::move(int** matrix2D, int pacman_x, int pacman_y){
...
void Pinky::moveJail(int** matrix2D){
	if (jail_time == 0 && jail){
...
void Pinky::moveChase(int** matrix2D, int pacman_x, int pacman_y){
	float x = pacman_x - xpoz;
	float y = pacman_y - ypoz;
...
void Pinky::moveLeft(int** matrix2D){
	xpoz = xpoz-ACT_RATE;
}

void Pinky::moveRight(int** matrix2D){
	xpoz = xpoz+ACT_RATE;
}

void Pinky::moveUp(int** matrix2D){
	ypoz = ypoz+ACT_RATE;
}

void Pinky::moveDown(int** matrix2D){
	ypoz = ypoz-ACT_RATE;
}
void Pinky::timer(){
	if(state > 0)
